...
# Obs: Crea una sesion que guarda el id del usuario y una cookie que guarda la sesion
...
my $sth = $dbh->prepare("SELECT id FROM usuario WHERE login_usuario=? AND login_clave=?");
